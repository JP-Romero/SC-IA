...
export default function PremiumView({ onUnlockPremium, isPremium, onNavigate }: PremiumViewProps) {
  ...
  const handleApplyPromo = () => {
    if (promoCode.trim().toUpperCase() === "SALUD100") {
      onUnlockPremium();
      setPromoMessage({ text: "✅ ¡Código 'SALUD100' aplicado con éxito! Has desbloqueado todos los servicios Premium totalmente gratis.", error: false });
      setPromoCode("");
    } else if (promoCode.trim() === "") {
      setPromoMessage({ text: "Introduce un código promocional válido.", error: true });
    } else {
      setPromoMessage({ text: "❌ Código inválido. Intenta con 'SALUD100' para pruebas asistenciales de evaluación.", error: true });
    }
  };
    ...
    <div className="flex flex-col min-h-screen pb-24 bg-gradient-to-b from-[#f5f8ff] to-[#f8fafc]">
      ...
      <div className="px-6 pt-5 pb-3 max-w-4xl mx-auto w-full">
        ...
        <p className="text-slate-400 text-xs mt-1 leading-relaxed">
          Potencia tu bienestar con beneficios exclusivos.
        </p>

        {isPremium && (
          <div className="mt-4 bg-emerald-50 border border-emerald-100 p-4 rounded-2xl text-xs text-emerald-800 font-bold flex items-center space-x-3">
            <div className="w-8 h-8 rounded-full bg-emerald-500 text-white flex items-center justify-center font-bold text-sm">🏆</div>
            <div>
              <span>¡Felicidades, Kenneth! Tu suscripción Premium está ACTIVA.</span>
              <p className="text-[10px] text-emerald-500 font-normal mt-0.5">Acceso libre ilimitado concedido. Próxima renovación: 2027.</p>
            </div>
          </div>
        )}

        <h3 className="font-display font-bold text-base text-slate-800 mt-6 mb-4">
          Elige tu plan
        </h3>
      </div>
      ...
      <main className="px-6 flex-1 space-y-6 max-w-4xl mx-auto w-full">
        ...
        <div className="bg-white rounded-2xl p-4 border border-slate-100 flex items-center space-x-3.5 relative overflow-hidden shadow-sm">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-0.5">
              Privacidad y seguridad primero. Información médica encriptada.
            </p>
          </div>
        </div>

        {/* PROMO CODES SEGMENT */}
        <div className="bg-slate-100/50 rounded-2xl p-4.5 border border-slate-200/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-3">
          <div className="flex items-center space-x-3">
            <Ticket className="w-5 h-5 text-blue-600 shrink-0" />
            <span className="text-xs font-semibold text-slate-700">¿Tienes un código promocional?</span>
          </div>
          ...
          <div className="flex gap-2 items-center w-full sm:w-auto">
            <input
              ...
              type="text"
              placeholder="Ej. SALUD100"
              value={promoCode}
              onChange={(e) => setPromoCode(e.target.value)}
              className="flex-1 sm:w-32 bg-white rounded-xl py-2 px-3 border border-slate-200 outline-none text-xs text-slate-800 font-mono"
            />
            <button
            ...
            >
              {checkoutSuccess ? (
                <div className="text-center py-6 space-y-4">
                  ...
                  <h3 className="font-display font-medium text-2xl text-slate-950">Suscripción Activa</h3>
                  <p className="text-xs text-slate-500 max-w-[260px] mx-auto leading-relaxed">
                    ¡Gracias! Tu pago de **{checkoutPlan.price}** para **{checkoutPlan.name}** se procesó con éxito de forma segura.
                  </p>
                  <p className="text-[10px] text-slate-400 font-mono">ID de Transacción: TXN-{Math.floor(Math.random() * 89999 + 10000)}</p>
                </div>
              ) : (
                <form onSubmit={handleCheckoutSubmit} className="space-y-4 text-left">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Número de Tarjeta</label>
                    <div className="relative">
                      <input
                        ...
                        required
                        className="w-full text-slate-800 bg-slate-50 rounded-xl py-2.5 px-3.5 pl-10 border border-slate-200 outline-none text-xs font-mono"
                      />
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      <label className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Vence</label>
                      <input
                        ...
                        required
                        className="w-full text-slate-800 bg-slate-50 rounded-xl py-2.5 px-3.5 border border-slate-200 outline-none text-xs font-mono"
                      />
                    </div>
                    <div className="space-y-1.5">
                  ...
                  >
                    <span>{isProcessingCheckout ? "Procesando pago encriptado..." : `Pagar ${checkoutPlan.price}`}</span>
                  </button>

                  <div className="text-[10px] text-slate-400/80 text-center leading-normal">
                    🔐 Conexión segura PCI-DSS v4.0. Sus datos bancarios no se guardan.
                  </div>
                </form>
              )}
            </motion.div>
