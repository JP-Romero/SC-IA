...
>>>>>>> a9f15842beb3b9e1722972d28cdcd44c5b81f20c
      ...
      <div className="px-6 pt-2 pb-3 z-10 relative">
        ...
        <p className="text-[#64748b] text-[13px] mt-1.5 leading-relaxed max-w-[280px]">
          Encuentra hospitales y centros<br />cercanos a ti.
        </p>

        {/* Location dropdown pill */}
        <div className="mt-4 inline-flex items-center gap-2 bg-white border border-[#e2e8f0] rounded-full px-4 py-2.5 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
          <svg viewBox="0 0 24 24" fill="none" stroke="#64748b" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 shrink-0">
            <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" />
            <circle cx="12" cy="10" r="3" />
          </svg>
          <span className="text-[13px] font-medium text-[#334155]">Granada, Nicaragua</span>
      ...
      <div className="px-0 relative z-10">
        <div
          className="relative overflow-hidden mx-3 rounded-[24px]"
          style={{
            height: "340px",
        ...
        >
          ...
          <div className="absolute z-10 select-none pointer-events-none" style={{ top: "38%", left: "28%", transform: "translate(-50%, -50%)" }}>
            <span className="text-[16px] font-bold text-[#334155]/40 tracking-wide" style={{ fontFamily: "'Inter', sans-serif" }}>Granada</span>
          </div>
          ...
          <AnimatePresence>
            {selectedCenter && (
              ...
              >
                <div className="bg-white rounded-2xl p-3 shadow-[0_8px_24px_rgba(0,0,0,0.1)] border border-slate-100">
                  <h4 className="text-[13px] font-bold text-[#0f172a] leading-tight">{selectedCenter.name}</h4>
                  <p className="text-[11px] text-[#64748b] mt-0.5">{selectedCenter.type}</p>
                  <div className="flex items-center gap-3 mt-1.5">
                    ...
                    <span className="text-[10px] font-medium text-[#64748b]">{selectedCenter.distance}</span>
                  </div>
                </div>
              </motion.div>
            )}
          ...
          <div className="absolute right-3 flex flex-col gap-2 z-20" style={{ bottom: "16px" }}>
            <button className="w-10 h-10 bg-white rounded-full shadow-[0_2px_10px_rgba(0,0,0,0.08)] border border-[#e2e8f0] flex items-center justify-center text-[#475569] active:scale-95 transition-transform">
      ...
      {/* ═══════════════ LIST SECTION ═══════════════ */}
      <div className="px-6 pt-6 z-10 relative flex-1">
        {/* Section header */}
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-[16px] font-bold text-[#0f172a] tracking-tight" style={{ fontFamily: "'Inter', sans-serif" }}>Cercanos a ti</h3>
          <button className="text-[13px] font-semibold text-[#2563eb]">Ver todos</button>
        </div>
          ...
          {filteredCenters.slice(0, 3).map((hc) => {
            ...
            return (
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                onClick={() => setSelectedCenter(hc)}
                className="w-full text-left flex items-center justify-between bg-white rounded-[20px] px-4 py-3.5 transition-all"
                style={{
                  border: isSelected ? "1.5px solid rgba(37,99,235,0.2)" : "1.5px solid #f1f5f9",
              ...
              >
                <div className="flex items-center gap-3.5">
                  ...
                  <div className="min-w-0">
                    <h4 className="text-[14px] font-semibold text-[#0f172a] leading-tight truncate">{hc.name}</h4>
                    <p className="text-[11.5px] text-[#94a3b8] mt-0.5">{hc.type}</p>
                ...
                <div className="shrink-0 text-right ml-3 flex flex-col items-end gap-0.5">
                  <span className="text-[13px] font-semibold text-[#334155]">{hc.distance}</span>
...
>>>>>>> a9f15842beb3b9e1722972d28cdcd44c5b81f20c
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className={`flex items-center gap-1 px-3 py-2 rounded-full text-[11px] font-semibold transition-all ${
